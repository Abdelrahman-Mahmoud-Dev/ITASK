<template>
    <v-card :flat="$route.name !== 'noteView'" class="mb-5 pa-3">
        ...
        <v-card-text>
          <strong v-if="$route.name === 'teacherQuestions'">{{$t('questions')}} : </strong>
          {{displayNote.body}}
        </v-card-text>
        <div v-if="$route.name === 'teacherQuestions' && $route.params.action !== 'newRequests'">
          <strong v-if="$route.name === 'teacherQuestions'" class="mx-5">{{$t('answers')}}</strong>
          <v-card-text v-for="n in 3" :key="n">{{displayNote.body}}</v-card-text>
          <audio controls class="my-5">
            <source src="../../assets/audio.mp3" type="audio/mpeg">
            Your browser does not support the audio element.
          </audio>
        </div>
        <video v-if="$route.params.action === 'video'" width="100%" controls autoplay loop>
            <source src="../../assets/vid.mp4">
        </video>
        <v-img v-else :src="require(`@/assets/icons/${displayNote.image}`)" max-height="300"></v-img>
        <div v-if="$route.name !== 'noteView' && $route.name !== 'teacherQuestions' && $route.params.action !== 'video'">
            <v-card-text v-for="n in 3" :key="n">{{displayNote.body}}</v-card-text>
        </div>
        ...
        <v-card-actions v-if="$route.params.action !== 'newRequests'">
            <div>
                ...
                <v-menu bottom>
                  <template v-slot:activator="{ on, attrs }">
                  <v-btn :disabled="!bought && !$route.path.includes('profile')" v-bind="attrs" v-on="on" small text>{{randomNumber()}}<v-icon small class="mx-1" color="primary">mdi-share-variant</v-icon></v-btn>
                  </template>
