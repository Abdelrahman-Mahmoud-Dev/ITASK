<template>
    <v-card :flat="$route.name !== 'noteView'" class="mb-5 pa-3">
        ...
        <v-card-text>
          <strong class="text-h6">{{$t('questionDetails')}}</strong>
          <br>
          {{displayNote.body}}
          {{displayNote.body}}
        </v-card-text>

        <v-img :src="require(`@/assets/icons/${displayNote.image}`)" max-height="300"></v-img>
        <div v-if="bought || $route.params.action !== 'answers'">
            <strong class="text-h6">{{$t('answerDetails')}}</strong>
            <br>
            <v-card-text v-for="n in 4" :key="n">{{displayNote.body}}</v-card-text>
            <audio controls class="my-5">
            <source src="../../assets/audio.mp3" type="audio/mpeg">
                Your browser does not support the audio element.
            </audio>
            <v-img :src="require(`@/assets/icons/physics.svg`)" max-height="300"></v-img>
        </div>
        ...
        <v-card-actions>
            <div v-if="bought || $route.params.action !== 'answers'">
                ...
                <v-menu bottom>
                  ...
                  <v-btn :disabled="!bought && !$route.path.includes('profile') && !$route.path.includes('teacher')" v-bind="attrs" v-on="on" small text>{{randomNumber()}}<v-icon small class="mx-1" color="primary">mdi-share-variant</v-icon></v-btn>
                  </template>
                ...
                >
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn :disabled="!bought && !$route.path.includes('profile')" small text v-bind="attrs" v-on="on">{{randomNumber()}}<v-icon small class="mx-1" color="primary">mdi-gift-outline</v-icon></v-btn>
                  </template>
            ...
            >
                <template v-slot:activator="{ on, attrs }">
                    <v-btn v-if="!bought && $route.params.action === 'answers'" color="primary" small elevation="0" v-bind="attrs" v-on="on">{{$t('getAnawer')}}</v-btn>
                </template>

                <v-card class="pt-5">
                    <h2 class="extra-font text-center mb-3">{{$t('getAnswerNote')}}</h2>
        ...
        <v-divider></v-divider>
        <v-card-text v-if="comments.length && bought  || $route.params.action !== 'answers'">
