			...
			<v-row>
				...
				<v-col cols="12" sm="6">
					...
					<v-text-field 
						...
						v-if="
							($route.params.action === 'bookingTeacher' ||
							$route.params.action === 'bookingClass') && $route.query.type == 'group'
						"
						hide-details
						:prefix="$t('students') + ':'"
						class="mt-1"
				...
				<v-col cols="12" sm="6" v-if="$route.params.action === 'bookingClass'">
					<v-text-field
						:readonly="$route.params.action === 'bookingClass'"
				...
				<v-col cols="12" v-for="(item, i) in materials" :key="item.id">
					<v-card class="pa-3">
						<div class="text-h6 primary--text mb-2">
							{{ item.subj }}, {{ item.lesson }},
							<router-link to="/teacher">{{ item.teacher }}</router-link>
						</div>
						<div class="mb-2">
							<h5>{{ new Date() | dateDisplay }} - {{ new Date() | dateDisplay }}</h5>
						</div>
						<div>{{ item.body }}</div>
					...
					<v-col cols="12" sm="6" v-for="item in materials" :key="item.id">
						<v-menu bottom v-if="$route.params.action === 'myClasses'">
							...
							<v-list>
								...
								<v-list-item>
									<v-list-item-icon
										><v-icon small color="primary">mdi-gift</v-icon></v-list-item-icon
									>
									<v-list-item-content>
										<v-dialog v-model="friendsDialogGift" width="700">
											<template v-slot:activator="{ on, attrs }">
					...
					<v-row class="mb-10">
						<v-col cols="12" sm="6" md="4" v-for="item in materials" :key="item.id">
							<v-card class="my-3 pa-2">
								<div class="text-h6 primary--text mb-2">
									{{ item.subj }}, {{ item.lesson }}, {{ item.teacher }}
								</div>
								<v-rating
