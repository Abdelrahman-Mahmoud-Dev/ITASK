<template>
    <div class="complete-reg-form px-1">
        <v-form
        ref="completeRegForm"
        v-model="valid"
        class="pt-2"
        >
            ...
            <div class="my-3">
              <v-btn @click="openImgUploader" text large color="primary"><v-icon>mdi-account-circle</v-icon> {{$t('profileImage')}}</v-btn>
              <v-btn @click="openImgUploader" text large color="primary"><v-icon>mdi-image</v-icon> {{$t('coverImage')}}</v-btn>
              <v-btn @click="openImgUploader" text large color="primary"><v-icon>mdi-file-pdf-box</v-icon> {{$t('resume')}}</v-btn>
            </div>

            <div>
              <v-img v-for="(item, i) in images" :key="item.href" :src="item.href" class="d-inline-block rounded-lg mx-1" height="50" width="50" gradient="to top right, rgba(100,115,201,.33), rgba(25,32,72,.7)">
...
export default {
  ...
  methods: {
    ...
    onFileChanged (e) {
      ...
      images.forEach(item => this.images.push({ href: URL.createObjectURL(item) }))
    },
    saveData () {
