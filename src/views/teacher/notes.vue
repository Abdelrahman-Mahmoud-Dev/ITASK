    ...
    <div>
        ...
        <div v-if="$route.params.action === 'createNote'">
          ...
          >
              <template v-slot:activator="{ on, attrs }">
                  <v-btn color="primary" class="px-15 mt-3 d-block mx-auto" large elevation="0" v-bind="attrs" v-on="on">{{$t('submit')}}</v-btn>
              </template>
        ...
        <div v-else>
              ...
              <v-row>
                  ...
                  <v-menu bottom v-if="$route.params.action === 'myNotes'">
                    ...
                    <v-list>
                      ...
                      <v-btn text @click="$route.params.action === 'myNotes' ? showNoteView = true : showNote = true">{{$t('grade') + Math.round(Math.random() * 13)}}, {{item.subj}}, {{item.lesson}},</v-btn>
                      <v-btn text @click="$router.push('/teacher')" class="px-0">{{item.teacher}}</v-btn>
                  </v-col>
              </v-row>
            </v-card>
            ...
            <div v-if="showNoteView" class="mt-5">
                <v-btn @click="showNoteView = false" text color="primary"><v-icon>mdi-keyboard-return</v-icon>{{$t('return')}}</v-btn>
