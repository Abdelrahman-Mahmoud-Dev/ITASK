                            ...
                            <v-row no-gutters>
                                ...
                                <v-col class="grey lighten-2 pa-2" cols="12" sm="4">
                                    ...
                                    <div v-if="selectedFriends.length">
                                        ...
                                        <v-btn small elevation="0" @click="invitationDialog = false" color="primary">Select</v-btn>
                                    </div>
                                </v-col>
                            </v-row>
                            </v-card-text>
        ...
        <div v-else>
            ...
            <div v-if="showNote" class="mt-5">
                ...
                <div v-if="$route.params.action === 'newRequests'">
                    <v-row class="mb-10">
                        <v-col cols="12" sm="6" md="4" v-for="(item, i) in materials" :key="item.id">
                            <v-card class="my-3 pa-2">
                                <div class="text-h6 primary--text mb-2">{{$t('grade') + 10}}, {{item.subj}}, {{item.lesson}}, {{item.teacher}}</div>
                                <div class="mb-2"><h5>{{ new Date() | dateDisplay }} - {{new Date() | dateDisplay}}</h5></div>
                                <div>{{displaySubjects[0].body}}</div>
                                <v-divider class="my-3"></v-divider>
                                <v-card-actions>
                                    <v-btn @click="materials.splice(i, 1)" small tile elevation="0" outlined class="px-10" color="primary">{{$t('reject')}}</v-btn>
                                    <v-spacer></v-spacer>
                                    <v-btn @click="materials.splice(i, 1)" small tile elevation="0" class="px-10" color="primary">{{$t('accept')}}</v-btn>
                                </v-card-actions>
                            </v-card>
                        </v-col>
                    </v-row>
