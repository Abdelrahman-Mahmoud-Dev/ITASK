<template>
    <div>
        <div class="profile-banner">
            <v-img height="400" :src="userExtras.cover ? userExtras.cover : require('../../assets/bg.jpg')" width="98%" class="mx-auto rounded-b-lg">
            </v-img>
            <div class="user-info">
                <v-avatar size="300">
                ...
                <p class="white--text">Lorem ipsum dolor sit amet consectetur adipisicing.</p>
                <div>
                  <v-btn class="mb-2 mx-1" color="primary" @click="$router.push('/addStory')"><v-icon left>mdi-plus-circle</v-icon>{{$t('addStory')}}</v-btn>
                  <v-btn class="mb-2" color="white" @click="$router.push('/teacher/settings')">{{$t('editProfile')}}</v-btn>
                </div>
            </div>
            <div class="profile-nav d-flex flex-wrap justify-space-between">
                <div class="d-flex flex-wrap">
                    <div v-for="item in items" :key="item.text">
                        <router-link v-if="item.type === 'link'" exact-active-class active-class="active-route" :to="'/profile' + item.route" class="mx-2 pa-2 font-weight-bold text-decoration-none white--text">
                            {{item.text}}
                        </router-link>
                        <div v-else :class="$route.path.includes(item.route) ? 'active-route' : ''">
                            <v-menu offset-y>
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn small color="white" class="font-weight-bold" text v-bind="attrs" v-on="on">{{item.text}} <v-icon>mdi-menu-down</v-icon></v-btn>
                            </template>
                            <v-list>
                                <v-list-item
                                color="primary"
                ...
                <v-col cols="12" sm="4" md="2">
                    <v-list class="sticky-side secondary--text">
                        ...
                        <v-list-item>
                            ...
                            <v-list-item-content>
                                <v-list-item-title>{{$t('math')}}</v-list-item-title>
                            </v-list-item-content>
                        </v-list-item>
                        <v-list-item>
                            <v-list-item-icon>
                            ...
                            <v-list-item-content>
                                <v-list-item-title>{{$t('grade')}} 2nd {{$t('secondary')}}</v-list-item-title>
                            </v-list-item-content>
                        </v-list-item>
                        <v-list-item>
                            <v-list-item-icon>
                        ...
                        <v-list-item>
                            <v-list-item-content>
                              <v-list-item-title>{{$t('grade')}} 10, 1500 {{$t('students')}}</v-list-item-title>
                            </v-list-item-content>
                        </v-list-item>

                        <v-list-item>
                            <v-list-item-content>
                              <v-list-item-title>{{$t('grade')}} 11, 1750 {{$t('students')}}</v-list-item-title>
                            </v-list-item-content>
                        </v-list-item>
                    </v-list>
                </v-col>
    ...
    items () {
      return [
        ...
        // },
        {
          ...
          route: '/calendar',
          links: [
            { link: 'myEvents', text: this.$t('myEvents') }
          ]
        },
        {
