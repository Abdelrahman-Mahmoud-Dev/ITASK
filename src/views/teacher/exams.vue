<template>
    <div>
      ...
      <div v-if="$route.params.action === 'createExam' || ($route.params.action === 'examRequest' && showExamForm)">
          <v-row v-if="$route.params.action === 'createExam'">
              <v-col cols="12" sm="6">
                <v-text-field solo readonly prepend-inner-icon="mdi-book" :value="displaySubjects[0].subj"></v-text-field>
              </v-col>
      ...
      <div v-else>
        ...
        <v-card class="pa-5 my-5" v-if="$route.params.action === 'examRequest'">
          <div v-for="(item, i) in displaySubjects" :key="item.id" class="my-5 d-flex justify-space-between">
            <div>
              <!-- @click="orderDialog = false; showNote = true; currentExam = `${$t('grade') + '10'}, ${item.subj}, ${item.lesson}, ${item.level}, ${item.teacher}`;" -->
            <v-btn large
              small text elevation="0">{{$t('grade') + '10'}}, {{item.subj}}, {{item.lesson}}, {{item.level}},</v-btn>
              <v-btn large text class="px-0" @click="$router.push('/teacher')">{{item.teacher}}</v-btn>
            </div>
            <div class="px-15">
              <v-btn elevation="0" large color="primary" class="mx-5 px-15" @click="showExamForm = true">{{$t('accept')}}</v-btn>
              <v-btn large outlined color="primary" class="px-15" @click="displaySubjects.splice(i, 1)">{{$t('reject')}}</v-btn>
            </div>
          </div>
        </v-card>
        ...
        <v-card v-if="$route.params.action === 'myExams'">
          <v-row v-show="!showNote" class="my-5">
                <v-col cols="12" sm="6" v-for="(item) in displaySubjects" :key="item.id">
                  ...
                  <v-btn
                  @click="orderDialog = false; showNote = true; currentExam = `${item.subj}, ${item.lesson}, ${item.level}, ${item.teacher}`;"
                  small text elevation="0">
                  {{item.subj}}, {{item.lesson}}, {{item.level}},
                  </v-btn>
                  <v-btn small text class="px-0" @click="$router.push('/teacher')">{{item.teacher}}</v-btn>
                </v-col>
          </v-row>
        </v-card>
